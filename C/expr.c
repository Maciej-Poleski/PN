...
static char const * expr_parse_factor(struct expr * expr, char const * text) {
    ...
    if (shift) {
        ...
        *left = *expr;
        ...
        expr->binary.right = (struct expr *) malloc(sizeof(struct expr));
        text = expr_parse_factor(expr->binary.right, text+shift);
    }
    return text;
}

static char const * expr_parse_term(struct expr * expr, char const * text) {
    text = expr_parse_factor(expr, text);
    for (;;) {
        int shift = 0;
        sscanf(text, " *%n", &shift);
        if (shift) {
